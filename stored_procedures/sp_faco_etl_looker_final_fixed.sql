-- ##############################################################################
-- # Stored Procedure CORREGIDO v2 para Looker Studio - FACO ETL              #
-- # FIX: Campos NULL manejados con valores por defecto                        #
-- ##############################################################################
...
BEGIN
  ...
  SELECT
    ...
    asig.cliente,
    asig.telefono,
    COALESCE(asig.negocio, 'SIN_SERVICIO') AS SERVICIO,                    -- 🔧 FIX: NULL -> valor por defecto
    COALESCE(asig.tramo_gestion, 'SIN_SEGMENTO') AS SEGMENTO_GESTION,       -- 🔧 FIX: NULL -> valor por defecto
    COALESCE(asig.zona, 'SIN_ZONA') AS ZONA_GEOGRAFICA,                     -- 🔧 FIX: NULL -> valor por defecto
    COALESCE(asig.min_vto, DATE('1900-01-01')) AS VENCIMIENTO,              -- 🔧 FIX: NULL -> fecha por defecto
    ...
    cal.FECHA_TRANDEUDA,
    cal.DIAS_GESTION,
    COALESCE(cal.ARCHIVO, 'SIN_ARCHIVO') AS ID_ARCHIVO_ASIGNACION,          -- 🔧 FIX: NULL -> valor por defecto
  ...
  -- 3c. Gestiones Homologadas y Unificadas (BOT + HUMANO) - CORREGIDO
  ...
  WITH gestiones_bot AS (
    SELECT 
      ...
      COALESCE(management, 'SIN_MANAGEMENT') AS management,                   -- 🔧 FIX: NULL -> valor por defecto
      '' AS sub_management,
      COALESCE(compromiso, '') AS compromiso,                                 -- 🔧 FIX: NULL -> valor por defecto
  ...
  -- ==================================================================
  -- 4. CREACIÓN DE TABLAS FACT SI NO EXISTEN
  -- ==================================================================
  
  -- Crear tabla faco_dash_asignacion_universo si no existe
  ...
  -- Crear tabla faco_dash_gestion_agregada si no existe
  ...
  -- Crear tabla faco_dash_recupero_atribuido si no existe
  ...
  CLUSTER BY CARTERA, CANAL_ATRIBUIDO;

  -- Crear tabla faco_dash_kpis_ejecutivos si no existe
  ...
  CLUSTER BY CARTERA, CANAL;

  -- ==================================================================
  -- 5. TABLA FACT 1: UNIVERSO DE ASIGNACIONES (Para Looker Studio)
  -- ==================================================================
  ...
  -- ==================================================================
  -- 6. TABLA FACT 2: GESTIÓN DIARIA AGREGADA (Para Looker Studio)
  -- ==================================================================
  ...
  -- ==================================================================
  -- 7. TABLA FACT 3: RECUPERO ATRIBUIDO (Para Looker Studio)
  -- ==================================================================
  ...
  -- ==================================================================
  -- 8. TABLA FACT 4: KPIs EJECUTIVOS CONSOLIDADOS
  -- ==================================================================
  ...
  -- ==================================================================
  -- 9. LOGGING FINAL Y ESTADÍSTICAS
  -- ==================================================================
