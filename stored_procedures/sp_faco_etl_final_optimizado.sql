-- ##############################################################################
-- # Stored Procedure FACO ETL - Versión Final Optimizada                     #
-- # Combina mejoras técnicas + correcciones de lógica de negocio              #
-- ##############################################################################
...
BEGIN
  ...
  -- ==================================================================
  -- 2. IDEMPOTENCIA - LIMPIEZA SEGURA
  -- ==================================================================
  EXECUTE IMMEDIATE FORMAT("DELETE FROM `%s` WHERE FECHA_ASIGNACION BETWEEN ? AND ?", v_tabla_asignacion) 
    USING p_fecha_inicio, p_fecha_fin;
  EXECUTE IMMEDIATE FORMAT("DELETE FROM `%s` WHERE FECHA_SERVICIO BETWEEN ? AND ?", v_tabla_gestion) 
    USING p_fecha_inicio, p_fecha_fin;
  EXECUTE IMMEDIATE FORMAT("DELETE FROM `%s` WHERE FECHA_PAGO BETWEEN ? AND ?", v_tabla_recupero) 
    USING p_fecha_inicio, p_fecha_fin;
  EXECUTE IMMEDIATE FORMAT("DELETE FROM `%s` WHERE FECHA_CALCULO BETWEEN ? AND ?", v_tabla_kpis) 
    USING p_fecha_inicio, p_fecha_fin;

  -- ==================================================================
  -- 3. TABLAS TEMPORALES BASE
  -- ==================================================================
  
  -- 3a. Universo de Asignaciones (Sin cambios - ya optimizado)
  ...
  -- 3b. Deuda Completa con Fechas Construidas (Mejorado)
  ...
  WITH deuda_con_fecha AS (
    SELECT 
      ...
      archivo,
      creado_el,
      -- Extraer fecha del nombre del archivo mejorado
      CASE 
  ...
  -- 3c. Gestiones Homologadas CORREGIDO - Unir BOT + HUMANO correctamente
  ...
  SELECT
    ...
    -- Operador final con homologación mejorada
    CASE 
    ...
    -- Homologación de respuestas mejorada con COALESCE
    CASE
    ...
    -- Flag de compromiso/PDP mejorado
    CASE
      WHEN g.canal_origen = 'BOT' THEN 
    ...
    -- Contacto efectivo mejorado
    CASE 
      WHEN UPPER(COALESCE(g.management, '')) LIKE '%CONTACTO_EFECTIVO%' 
        OR UPPER(COALESCE(g.management, '')) LIKE '%EFECTIVO%' THEN 1
      ELSE 0 
    END AS es_contacto_efectivo
    
  FROM gestiones_unificadas g
  
  -- Homologación BOT con COALESCE mejorado
  LEFT JOIN `mibot-222814.BI_USA.homologacion_P3fV4dWNeMkN5RJMhV8e_voicebot` AS h_bot 
    ON g.canal_origen = 'BOT' 
    AND COALESCE(g.management, '') = COALESCE(h_bot.bot_management, '')
    AND COALESCE(g.sub_management, '') = COALESCE(h_bot.bot_sub_management, '')
    AND COALESCE(g.compromiso, '') = COALESCE(h_bot.bot_compromiso, '')
  
  -- Homologación HUMANO con COALESCE mejorado
  LEFT JOIN `mibot-222814.BI_USA.homologacion_P3fV4dWNeMkN5RJMhV8e_v2` AS h_call 
    ON g.canal_origen = 'HUMANO' 
    AND COALESCE(g.management, '') = COALESCE(h_call.management, '')
  
  -- Homologación USUARIOS con COALESCE mejorado
  LEFT JOIN `mibot-222814.BI_USA.homologacion_P3fV4dWNeMkN5RJMhV8e_usuarios` AS h_user 
    ON g.canal_origen = 'HUMANO' 
    AND COALESCE(g.nombre_agente, '') = COALESCE(h_user.usuario, '');

  -- ==================================================================
  -- 4. TABLA FACT 1: UNIVERSO DE ASIGNACIONES
  -- ==================================================================
  ...
  -- ==================================================================
  -- 5. TABLA FACT 2: GESTIÓN DIARIA AGREGADA (CORREGIDO)
  -- ==================================================================
  ...
  (
    ...
    -- Métricas de CLIENTES ÚNICOS (CORREGIDO - agregamos el campo faltante)
  ...
  WITH gestiones_en_contexto AS (
    SELECT
      ...
      -- Flag de primera vez por día mejorado
      ROW_NUMBER() OVER(
  ...
  SELECT
    ...
    -- Métricas de CLIENTES ÚNICOS (CORREGIDAS)
    ...
    -- KPIs Calculados con SAFE_DIVIDE mejorado
  ...
  -- ==================================================================
  -- 6. TABLA FACT 3: RECUPERO ATRIBUIDO (Mejorado)
  -- ==================================================================
  ...
  SELECT
    ...
    -- Score de efectividad de atribución mejorado
    CASE
  ...
  -- ==================================================================
  -- 7. TABLA FACT 4: KPIs EJECUTIVOS CONSOLIDADOS (RESTAURADO)
  -- ==================================================================
  ...
  WITH kpis_por_dimension AS (
    SELECT
      p_fecha_inicio AS FECHA_CALCULO,
  ...
  SELECT
    ...
    -- KPIs calculados con SAFE_DIVIDE
