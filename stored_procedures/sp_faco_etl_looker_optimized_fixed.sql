-- ##############################################################################
-- # Stored Procedure CORREGIDO para Looker Studio - FACO ETL                 #
-- # Genera tablas fact optimizadas para dashboards con clustering BigQuery    #
-- # FIX: Tipos de datos compatibles en UNION ALL                              #
-- ##############################################################################
...
BEGIN
  ...
  WITH gestiones_bot AS (
    SELECT 
      ...
      CAST(0 AS FLOAT64) AS monto_compromiso,  -- 🔧 FIX: Tipo explícito FLOAT64
      CAST(NULL AS DATE) AS fecha_compromiso,  -- 🔧 FIX: Tipo explícito DATE
      'BOT' AS canal_origen
  ...
  gestiones_humano AS (
    SELECT 
      ...
      CAST(COALESCE(monto_compromiso, 0) AS FLOAT64) AS monto_compromiso,  -- 🔧 FIX: Tipo explícito FLOAT64
      CAST(fecha_compromiso AS DATE) AS fecha_compromiso,                  -- 🔧 FIX: Tipo explícito DATE
